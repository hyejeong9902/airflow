...
def make_predict_data():
    ...
    try:
        ...
        # 매일 자정 수행된다고 가정했을 때(schedule_interval="@daily") 
        # (as-is) 주치의소견 테이블에서 LAST_CHANGE_ILSI가 하루 전인 경우(주치의 소견에 장해등급 정보가 있으면 신청을 한 사람으로 가정)
        with db_connect.cursor() as cur:
            AAA200MT = pd.read_sql_query('SELECT * FROM "AAA200MT" WHERE "LAST_CHANGE_ILSI"=(CURRENT_DATE - 1)', db_connect)
            # 빈 데이터셋 체크
            if len(AAA200MT)==0:
                print("예측 대상자가 없습니다.")
                db_connect.close()
            ...
            base_query = '''SELECT * FROM "{table}" WHERE "WONBU_NO" IN (SELECT "WONBU_NO" FROM "AAA200MT" WHERE "LAST_CHANGE_ILSI" = (CURRENT_DATE - 1))'''
